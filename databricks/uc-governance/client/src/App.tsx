...
import { UCObjectsPage } from './pages/lakebase/UCObjectsPage';
...
function NavLinks({ className, linkClass, onClick }: { className?: string; linkClass: NavLinkClassFn; onClick?: () => void }) {
  return (
    <nav className={className}>
      ...
      <NavLink to="/uc-objects" className={linkClass} onClick={onClick}>
        UC Objects
      </NavLink>
    </nav>
  );
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <header className="border-b px-4 md:px-6 py-3 flex items-center gap-4">
        <h1 className="text-lg font-semibold text-foreground">uc-governance</h1>
...
const router = createBrowserRouter([
  {
    ...
    children: [
      ...
      { path: '/uc-objects', element: <UCObjectsPage /> },
    ],
  },
]);
    ...
    <div className="max-w-2xl mx-auto space-y-6 mt-8">
      <div className="text-center">
        ...
        <p className="text-lg text-muted-foreground">
          Manage and orchestrate UC objects with GitOps workflows
        </p>
      </div>
